...
type EditableCalendarEvent = {
  ...
  endsAtInput: string;
  isAllDay: boolean;
  color: CalendarEventColorValue;
};

type EditCalendarEventModalProps = {
...
export function EditCalendarEventModal({
  ...
  const [color, setColor] = useState<CalendarEventColorValue>(event.color);
  ...
  return (
    <>
      <button
        type="button"
        onClick={openModal}
        className="cursor-pointer rounded-md border border-[#d9e2f3] bg-white px-2 py-1 text-[10px] font-semibold tracking-[0.08em] text-[#4e5e7a] uppercase hover:bg-[#f8faff]"
      >
        Edit
      </button>
        ...
        <form action={action} className="space-y-4 p-6">
          ...
          <label className="inline-flex items-center gap-2 text-sm text-[#4e5e7a]">
            <input
              ...
              className="h-4 w-4 rounded border-[#cad8ef]"
            />
            All day event
          </label>
